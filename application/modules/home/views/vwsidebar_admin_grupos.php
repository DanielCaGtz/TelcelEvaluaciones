	...
	<script>
		$(document).ready(function(){
			...
			$("#save_cambios").on("click",function(){
				...
				$.each($("#example1").find(".select_user_checkbox"),function(i,item){
					if($(item).prop("checked")){
						...
						temp[j++]=$(item).parent().parent().find(".fecha_from p").html();
						temp[j++]=$(item).parent().parent().find(".fecha_to p").html();
						users[x++]=temp;
					}
				});
				if(users.length>0){
					$.post(window.url.base_url+"home/ctrhome/guardar_edicion_grupos",{data:users},function(resp){
						...
						if(resp.success!==false){
							...
							$.each($("#example1").find(".select_user_checkbox"),function(i,item){
								if($(item).prop("checked")){
									...
									$(item).parent().parent().find(".fecha_from p").attr("data-original",$(item).parent().parent().find(".fecha_from p").html());
									$(item).parent().parent().find(".fecha_to p").attr("data-original",$(item).parent().parent().find(".fecha_to p").html());
								}
							});
							setTimeout(function(){
								$("#mensaje_error_edicion").hide();
			...
			$("#eliminar_usuario").on("click",function(){
				if(confirm("¿Realmente desea eliminar los grupos seleccionados?")){
					var temp=Array();
					$.each($("#example1").find(".select_user_checkbox"),function(i,item){
						if($(item).prop("checked")){
					...
					$.post(window.url.base_url+"home/ctrhome/eliminar_grupos",{data:temp},function(resp){
						resp=JSON.parse(resp);
						location.reload();
					});
				}
